    ...
    async processTick(tickData) {
        ...
        if (weather.rainfall > 50 || hasCrisisCondition) {
            ...
            const systemPrompt = `
                ...
                Predict if this zone is at risk of a major crisis (flash flooding, fire spread, structural collapse from earthquake, etc).
                If risk > 70%, return a JSON alert:
                { "type": "flood_warning" | "fire_warning" | "crisis_alert", "severity": "medium" or "high", "message": "One sentence about the specific crisis risk and location" }
                If risk is low, return: { "no_risk": true }
            `;

            const prediction = await this.promptAI(systemPrompt, JSON.stringify({ weather, sensors, abnormalConditions }));
