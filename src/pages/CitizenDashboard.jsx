...
export default function CitizenDashboard() {
    ...
    const handleCitySearch = async (e) => {
        ...
        setCityError(null);
        try {
            // 1. Fetch the static city map/graph data (for rendering the mini-map if needed)
            const data = await fetchCityData(searchCity);
            setCityData(data);
            fetchNews(searchCity);

            // 2. Fetch the LIVE PUBLISHED STATE from Supabase
            const { supabase } = await import('../services/supabaseClient.js');
            const normalizedName = searchCity.trim().toLowerCase();
            console.log(`[Dashboard] Fetching live state for: ${normalizedName}`);
            ...
            if (dbData) {
                console.log("[Dashboard] Loaded live city state:", dbData);

                // Populate the dashboard with the persistent state
                if (dbData.action_plan) {
                    setPublishedPlan(dbData.action_plan);
                }

                if (dbData.alerts && dbData.alerts.length > 0) {
                    setPublicAlerts(dbData.alerts);
                }

                // The most critical part: load the query engine feeder so CityBuddy has context
                // even if the user hasn't generated a twin locally.
                if (dbData.query_engine_feeder) {
                    setLatestTickData({
                        areaConfig: { params: dbData.query_engine_feeder.weatherAndEnvironment || {} },
                        abnormalConditions: [], // We could derive this from the feeder
                        weather: dbData.weather
                    });

                    // Construct a custom state for CityBuddy based on the persistent data
                    const persistentState = {
                        cityData: data,
                        sensorData: { abnormalConditions: [] },
                        areaParams: {
                            ...dbData.query_engine_feeder.weatherAndEnvironment,
                            ...dbData.query_engine_feeder.infrastructureAndTransport,
                            ...dbData.query_engine_feeder.populationAndCrisis
                        }
                    };

                    // We'll store this in a ref or state so handleChatSubmit can use it
                    // For now, setting latestTickData closely mimics it
                    setLatestTickData({
                        areaConfig: { params: persistentState.areaParams },
                        abnormalConditions: []
                    });
                }
            } else {
                console.log("[Dashboard] No published state found for this city. Waiting for AI Twin to publish.");
    ...
    useEffect(() => {
        const handleTick = (data) => {
            // Only update live tick data if we are actively running a simulation
            // In a pure dashboard view, we might rely more on the Supabase polling
        ...
        // Fallback polling: We still check localStorage in case Supabase is skipped during dev
        const checkPlan = () => {
            const planStr = localStorage.getItem('citybuddy_published_action_plan');
            if (planStr && !publishedPlan) {
                try {
